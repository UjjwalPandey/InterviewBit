...
public class Seats {
    ...
    private static int seats(String A){
        ...
        for(int i = 0; i < median; i++) {
            ans = (ans + points.get(median) - points.get(i) - ans1)%MOD;
            ans1++;
//            System.out.println("Median = "+median+"  ANs= "+ans+"  Ans1 = "+ans1+" points[median] = "+points.get(median)+"  i ="+i+"  points[i] = "+points.get(i));
        }
        ans1=1;
        for(int i = median + 1; i < points.size(); i++) {
            ans = (ans+points.get(i) - ( points.get(median) + ans1))%MOD;
            ans1++;
//            System.out.println("Median = "+median+"  ANs= "+ans+"  Ans1 = "+ans1+" points[median] = "+points.get(median)+"  i ="+i+"  points[i] = "+points.get(i));
        }
        return ans%MOD;
