...
public class CountRightTriangles {
    ...
    public static int solve(int[] A, int[] B) {
        HashMap<Integer, Integer> ma1 = new HashMap<>();
        HashMap<Integer, Integer> ma2 = new HashMap<>();

        for(int i=0;i<A.length;i++){
            ma1.put(A[i], ma1.getOrDefault(A[i],0)+1);
            ma2.put(B[i], ma2.getOrDefault(B[i],0)+1);
        }
        long cnt=0;
        for(int i=0;i<A.length;i++) {
            cnt+= ((ma1.get(A[i])-1)*(ma2.get(B[i])-1))%1000000007;
            cnt%=1000000007;
        }
