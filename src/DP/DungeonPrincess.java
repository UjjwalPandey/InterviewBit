...
public class DungeonPrincess {
    ...
    public static int calculateMinimumHP(int[][] A) {
        ...
        // entire table 
        for (int i = m-2; i >= 0; i--)
            dp[i][n-1] = Math.max(dp[i+1][n-1] - A[i][n-1], 1);
        for (int j = n-2; j >= 0; j--)
            dp[m-1][j] = Math.max(dp[m-1][j+1] - A[m-1][j], 1);
        ...
        for (int i=m-2; i>=0; i--)
        {
            for (int j=n-2; j>=0; j--)
            {
                int min_A_on_exit = Math.min(dp[i+1][j], dp[i][j+1]);
                dp[i][j] = Math.max(min_A_on_exit - A[i][j], 1);
            }
        }
