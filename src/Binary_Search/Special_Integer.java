/* Special Integer
    ...
    there is no subarray in A of size K with sum of elements greater than B.
...
public class Special_Integer {
    ...
    // Faulty Algo --> as Maximum value doesn't ensure maximum sum with lesser values in neighbourhood.
    public static int solve(int[] A, int B) {
        int max = Integer.MIN_VALUE;
        int max_pos = -1;
        for(int i=0; i<A.length; i++){
            max = Math.max(max, A[i]);
            if(max == A[i]) max_pos = i;
        }
        System.out.println("Max value = "+max+", Max Pos = "+max_pos);
        int l =max_pos-1, r = max_pos+1, sum = max, counter =1;
        while ((l >=0 || r < A.length)&& sum < B){
            if(l < 0){
                sum += A[r++];
            }else if (r > A.length ){
                sum += A[l--];
        ...
        if(counter == A.length){
            System.out.println("L = "+l+" R = "+r+" Counter = "+counter);
            System.out.println("All elements included");
            return A.length;
        }
        return counter-1;
    }
    ...
    public static int solve_Double_Loop(int[] A, int B) {
        ...
        for(int i=0; i < A.length; i++){
            ...
            for(int j=i; j< A.length; j++){
                ...
                if(sum > B) {
                    flag= true;
//                    System.out.println("I = "+i+" J = "+j+" Window = "+windowSize);
                    break;
                }
                counter++;
            }
//            System.out.println("Counter = "+counter);
            if(windowSize > counter && flag) {
//                System.out.println(windowSize);
                windowSize =counter;
                min = Math.min(counter, min);
//                System.out.println("i = "+i+" Window  = "+windowSize);
            }
        }
        if(min == Integer.MAX_VALUE) return A.length;
        return min;
    }
}
